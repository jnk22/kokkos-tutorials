...
double scatter_view_loop(Kokkos::View<int**> v, Kokkos::View<int*> r) {
  // EXERCISE: Create ScatterView before starting timer, assuming you could
  // reuse it in real code
  ...
  Kokkos::parallel_for(
      "Atomic Loop", v.extent(0), KOKKOS_LAMBDA(const int i) {
        auto r_a = scatter_r.access();
        // EXERCISE: get accessors
        for (int j = 0; j < v.extent(1); j++) r_a(v(i, j))++;
      });
  // EXERCISE: contribute back to original results view
  //  Wait for Kernel to finish before timing
  scatter_r.contribute_into(r);
  Kokkos::fence();
...
double atomic_loop(Kokkos::View<int**> v,
  ...
  Kokkos::parallel_for(
      "Atomic Loop", v.extent(0), KOKKOS_LAMBDA(const int i) {
        for (int j = 0; j < v.extent(1); j++) r(v(i, j))++;
      });
  // Wait for Kernel to finish before timing
  Kokkos::fence();
...
double openmp_loop(Kokkos::View<int**, Kokkos::HostSpace> v,
  ...
  Kokkos::View<int**, Kokkos::HostSpace> results(
      "Rdup", Kokkos::OpenMP().concurrency(), r.extent(0));
  ...
  Kokkos::parallel_for(
      "Duplicated Loop", Kokkos::RangePolicy<Kokkos::OpenMP>(0, v.extent(0)),
      KOKKOS_LAMBDA(const int i) {
        // Every thread contribues to its version of the vector
        int tid = omp_get_thread_num();
        for (int j = 0; j < v.extent(1); j++) results(tid, v(i, j))++;
      });
  // Contribute back to the single version
  Kokkos::parallel_for(
      "Reduce Loop", Kokkos::RangePolicy<Kokkos::OpenMP>(0, v.extent(0)),
      KOKKOS_LAMBDA(const int i) {
