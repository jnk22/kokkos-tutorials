...
int main(int argc, char *argv[]) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    // this in Kokkos programs. Allocate y, x vectors and Matrix A:
    // EXERCISE: For the inpatient only: replace std::malloc with
    ...
    auto A =
        static_cast<double *>(Kokkos::kokkos_malloc(N * M * sizeof(double)));
    ...
    // }
    Kokkos::parallel_for(
        "y_init", N, KOKKOS_LAMBDA(const int i) { y[i] = 1; });
    printf("y[0] = %f\n", y[0]);
    ...
    // }
    Kokkos::parallel_for(
        "x_init", M, KOKKOS_LAMBDA(const int i) { x[i] = 1; });
    printf("x[0] = %f\n", x[0]);
    ...
    // }
    Kokkos::parallel_for(
        "A_init", N, KOKKOS_LAMBDA(const int j) {
          for (int i = 0; i < M; ++i) {
            A[j * M + i] = 1;
          }
        });
    ...
    for (int repeat = 0; repeat < nrepeat; repeat++) {
      ...
      Kokkos::parallel_reduce(
          "Reduction", N,
          KOKKOS_LAMBDA(const int j, double &update) {
            double temp2 = 0;
      ...
      const double solution = static_cast<double>(N) * static_cast<double>(M);
