...
// To practice usage of subviews, by creating a subview of row j of matrix A
// within the parallel_reduce lambda body; then replace the multiplication A( j,
// : ) * x( : ) with row_j_A( : ) * x( : )
...
int main(int argc, char *argv[]) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    double Gbytes =
        1.0e-9 * static_cast<double>(sizeof(double) * (M + M * N + N));
