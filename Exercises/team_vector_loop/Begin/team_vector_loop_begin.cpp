...
// EXERCISE 6 Goal:
//   Convert to three-level team parallelism using team policy within the nested
//   loops.
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < argc; i++) {
    if ((strcmp(argv[i], "-N") == 0) || (strcmp(argv[i], "-Rows") == 0)) {
      ...
      printf(
          ...
          "2^num (default: 2^18 = 256*1024 )\n");
      printf(
          "  -Elements (-E) <int>:  exponent num, determines number of "
          "elements 2^num (default: 2^10 = 1024 )\n");
  ...
  Kokkos::initialize(argc, argv);
  {
      ...
      Kokkos::parallel_reduce(
          ...
          KOKKOS_LAMBDA(const member_type &teamMember, double &update) {
            ...
            Kokkos::parallel_reduce(
                ...
                [&](const int j, double &middleUpdate) {
                  double innerResult = 0;

                  Kokkos::parallel_reduce(
                      Kokkos::ThreadVectorRange(teamMember, M),
                      [&](const int i, double &innerUpdate) {
                        innerUpdate += A(e, j, i) * x(e, i);
                      },
                      innerResult);

                  middleUpdate += y(e, j) * innerResult;
                },
                middleResult);

            // EXERCISE: Replace team_rank check with Kokkos::single
            // construct.
            // if (teamMember.team_rank() == 0)
            //   update += y(e, j) * innerResult;
            single(Kokkos::PerTeam(teamMember),
                   [&]() { update += middleResult; });
          },
          result);
    ...
    printf(
        "  N( %d ) M( %d ) E( %d ) nrepeat ( %d ) problem( %g MB ) time( %g s "
        ") bandwidth( %g GB/s )\n",
...
void checkSizes(int &N, int &M, int &S, int &E, int &nrepeat) {
  // If S is undefined and N or M is undefined, set S to 2^18 or the bigger of N
  // and M.
  if (S == -1 && (N == -1 || M == -1)) {
    S = pow(2, 18);
    if (S < N) S = N;
